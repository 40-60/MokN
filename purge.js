...
async function purgeFiles(patterns = []) {
  ...
  if (!fs.existsSync(distPath)) {
    console.error(
      "❌ dist folder not found. Make sure to build the project first."
    );
    return;
  }
  ...
  const relativePaths = filesToPurge.map((file) =>
    path.relative(distPath, file).replace(/\\/g, "/")
  );
...
async function purgeSrcFiles(patterns = []) {
  ...
  const relativePaths = filesToPurge.map((file) =>
    path.relative(srcPath, file).replace(/\\/g, "/")
  );
  ...
  console.log("");
  console.log(
    `📊 Src Summary: ${successCount} successful, ${errorCount} errors`
  );
}
